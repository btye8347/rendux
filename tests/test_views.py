...
def test_view_config_service_loads_configured_views():
    ...
    assert {v["id"] for v in views} == {"home", "components", "services", "chat", "ops", "about"}
...
def test_views_json_route_lists_views():
    ...
    assert view_ids == {"home", "components", "services", "chat", "ops", "about"}
...
def test_services_resolve_workspace_kind_is_layout():
    service = ViewConfigService.from_yaml(CONFIG_PATH)

    ws = service.resolve_workspace("services")

    assert ws["kind"] == "layout"
    assert service.view_data("services")["services_table"]["title"] == "Service catalog"
